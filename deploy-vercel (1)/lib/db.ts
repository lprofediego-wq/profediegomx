...
async function vGet<T>(key: string): Promise<T | null> {
  ...
  } catch (e) { console.error('[db] vGet error:', e); return null }
}
async function vSet(key: string, val: unknown): Promise<void> {
  try {
    const { kv } = await import('@vercel/kv')
    await kv.set(key, val)
  } catch (e) { console.error('[db] vSet error:', e) }
}
async function vDel(key: string): Promise<void> {
  try {
    const { kv } = await import('@vercel/kv')
    await kv.del(key)
  } catch (e) { console.error('[db] vDel error:', e) }
}
async function vList(prefix: string): Promise<string[]> {
  try {
    ...
    return keys
  } catch (e) { console.error('[db] vList error:', e); return [] }
}
...
export async function upsertCourse(data: Partial<Course> & { id?: string; slug: string; title: string }): Promise<Course> {
  ...
  const idx = (await kGet<string[]>('courses-index')) ?? []
  if (!idx.includes(course.id)) { idx.push(course.id); await kSet('courses-index', idx) }
  return course
}
...
export async function getLessonsByCourse(courseId: string): Promise<Lesson[]> {
  ...
  return (lessons.filter(Boolean) as Lesson[]).sort((a, b) => a.position - b.position)
}
export async function upsertLesson(data: Omit<Lesson, 'created_at'> & { id?: string }): Promise<Lesson> {
  ...
  if (!ids.includes(lesson.id)) { ids.push(lesson.id); await kSet(`lessons-by-course:${lesson.course_id}`, ids) }
  return lesson
}

export async function hasAccess(userId: string, courseId: string): Promise<boolean> {
  return (await kGet(`access:${userId}:${courseId}`)) !== null
}
export async function grantAccess(userId: string, courseId: string, grantedBy: string): Promise<void> {
  const rec: CourseAccess = { id: crypto.randomUUID(), user_id: userId, course_id: courseId, granted_by: grantedBy, granted_at: new Date().toISOString(), expires_at: null }
  await kSet(`access:${userId}:${courseId}`, rec)
  const ids = (await kGet<string[]>(`access-by-user:${userId}`)) ?? []
  if (!ids.includes(courseId)) { ids.push(courseId); await kSet(`access-by-user:${userId}`, ids) }
}
...
export async function markLessonComplete(userId: string, lessonId: string): Promise<void> {
  ...
  const rec: LessonProgress = { id: existing?.id ?? crypto.randomUUID(), user_id: userId, lesson_id: lessonId, completed: true, watched_sec: existing?.watched_sec ?? 0, completed_at: new Date().toISOString() }
...
export async function seedIfNeeded(): Promise<void> {
  try {
    ...
    if (!existing) {
      const admin: StoredUser = { id: crypto.randomUUID(), email: adminEmail, full_name: 'Profe Diego', role: 'admin', is_active: true, avatar_url: null, password_hash: await hashPassword(adminPass), created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
      await createUser(admin)
    }
    await kSet('seeded', '1')
  } catch (e) { console.error('[db] seedIfNeeded error (non-fatal):', e) }
}
...
export async function getSubjectsByCourse(courseId: string): Promise<Subject[]> {
  ...
  return (subjects.filter(Boolean) as Subject[]).sort((a, b) => a.position - b.position)
}
export async function upsertSubject(data: Omit<Subject, 'created_at'> & { id?: string }): Promise<Subject> {
  ...
  const now = new Date().toISOString()
  const subject: Subject = { id: data.id ?? crypto.randomUUID(), course_id: data.course_id, title: data.title, description: data.description ?? null, position: data.position, created_at: (existing as Subject | null)?.created_at ?? now }
  await kSet(`subject:${subject.id}`, subject)
  const ids = (await kGet<string[]>(`subjects-by-course:${subject.course_id}`)) ?? []
  if (!ids.includes(subject.id)) { ids.push(subject.id); await kSet(`subjects-by-course:${subject.course_id}`, ids) }
  return subject
}
...
export async function upsertMaterial(data: Omit<LessonMaterial, 'created_at'> & { id?: string }): Promise<LessonMaterial> {
  const mat: LessonMaterial = { id: data.id ?? crypto.randomUUID(), lesson_id: data.lesson_id, title: data.title, url: data.url, type: data.type, created_at: new Date().toISOString() }
  await kSet(`material:${mat.id}`, mat)
  const ids = (await kGet<string[]>(`materials-by-lesson:${mat.lesson_id}`)) ?? []
  if (!ids.includes(mat.id)) { ids.push(mat.id); await kSet(`materials-by-lesson:${mat.lesson_id}`, ids) }
  return mat
}
...
export async function upsertExam(data: Omit<Exam, 'created_at'> & { id?: string }): Promise<Exam> {
  ...
  const now = new Date().toISOString()
  const exam: Exam = { id: data.id ?? crypto.randomUUID(), course_id: data.course_id, title: data.title, description: data.description ?? null, duration_min: data.duration_min ?? 60, pass_score: data.pass_score ?? 70, is_published: data.is_published ?? false, created_at: (existing as Exam | null)?.created_at ?? now }
  await kSet(`exam:${exam.id}`, exam)
  const ids = (await kGet<string[]>(`exams-by-course:${exam.course_id}`)) ?? []
  if (!ids.includes(exam.id)) { ids.push(exam.id); await kSet(`exams-by-course:${exam.course_id}`, ids) }
  return exam
}
...
export async function getQuestionsByExam(examId: string): Promise<ExamQuestion[]> {
  ...
  return (qs.filter(Boolean) as ExamQuestion[]).sort((a, b) => a.position - b.position)
}
export async function upsertQuestion(data: Omit<ExamQuestion, never> & { id?: string }): Promise<ExamQuestion> {
  const q: ExamQuestion = { id: data.id ?? crypto.randomUUID(), exam_id: data.exam_id, question: data.question, options: data.options, correct_index: data.correct_index, explanation: data.explanation ?? null, position: data.position }
  await kSet(`question:${q.id}`, q)
  const ids = (await kGet<string[]>(`questions-by-exam:${q.exam_id}`)) ?? []
  if (!ids.includes(q.id)) { ids.push(q.id); await kSet(`questions-by-exam:${q.exam_id}`, ids) }
  return q
}
...
export async function getAttemptsByUserExam(userId: string, examId: string): Promise<ExamAttempt[]> {
  ...
  return (attempts.filter(Boolean) as ExamAttempt[]).sort((a, b) => new Date(b.completed_at).getTime() - new Date(a.completed_at).getTime())
}
