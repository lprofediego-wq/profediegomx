...
// app/admin/page.tsx — Dashboard admin (sin Supabase)
import { listUsers, listCourses, countAllAccess, seedIfNeeded } from '@/lib/db'
import { Users, BookOpen, TrendingUp, UserCheck, ArrowRight } from 'lucide-react'
import Link from 'next/link'

export default async function AdminDashboardPage() {
  await seedIfNeeded()
  const [users, courses, totalAccess] = await Promise.all([
    listUsers(),
    listCourses(),
    countAllAccess(),
  ])
  ...
  const kpis = [
    { label: 'Total alumnos',     value: totalUsers,   icon: Users,      color: 'bg-blue-50 text-blue-600',     border: 'border-blue-100'    },
    { label: 'Alumnos activos',   value: activeUsers,  icon: UserCheck,  color: 'bg-emerald-50 text-emerald-600', border: 'border-emerald-100' },
    { label: 'Cursos publicados', value: totalCourses, icon: BookOpen,   color: 'bg-purple-50 text-purple-600', border: 'border-purple-100'  },
          ...
          {recentUsers.map(u => (
            <div key={u.id} className="px-5 py-3.5 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${u.role === 'admin' ? 'bg-brand-100 text-brand-700' : 'bg-slate-100 text-slate-500'}`}>
